# d） unrolling with a factor of 4
.data 
...
loop: 	
	...
	l.s       $f15, 8($t1)      	 # loading B[i-2] to $f15
	l.s       $f5,  12($t0)     	 # loading A[i-3] to $f5
	l.s       $f16, 12($t1)      	 # loading B[i-3] to $f16
	...
	mul.s     $f6, $f6, $f15    	 # calculate a[i-2]*b[i-2] -->f6
	add.s     $f7, $f7, $f6    	 # sum = sum + f6
	mul.s     $f5, $f5, $f16    	 # calculate a[i-3]*b[i-3] -->f
	add.s     $f7, $f7, $f5    	 # sum = sum + f5
				
   	addi      $t0, $t0, 16       	 # increment pointer for A[i-4]
        addi      $t1, $t1, 16        	 # increment pointer for B[i-4]     
        addi      $s0, $s0, -4		 # i=i-4
test:
        bgt       $s0, $zero, loop	 # branch:  continue if i>0
